...
/** A goal as generated by the event sim, before it's tied to a team id (that happens in competitions). */
...
export interface MatchResult {
  ...
  /** Every goal in play order, with scorer/assist, from the event sim. */
  goals: SimGoal[];
}
...
// ---- Event-based match sim -------------------------------------------------------------------
//
// Instead of sampling one whole-match scoreline, we play the match minute by minute: each side has a
// per-minute scoring intensity (its expected goals spread over 90'), modulated by game state, and
// each goal is attributed to a real player on the pitch. This gives causally-consistent scorers, and
// game-state feedback (leaders sit back, chasers push, level games go cagey late) reproduces
// comebacks, late goals and draw inflation that a single Poisson draw cannot. The closed-form
// Dixon-Coles above still computes the *displayed* pre-match win/draw/loss odds from the same lambdas.

/** Finishing propensity by position — chance a goal is scored by a player in that slot. */
const GOAL_WEIGHT: Partial<Record<Player['position'], number>> = {
  GK: 0.02, CB: 0.35, LB: 0.25, RB: 0.25, LWB: 0.3, RWB: 0.3,
  CDM: 0.4, CM: 0.75, CAM: 1.25, LM: 0.9, RM: 0.9, LW: 1.55, RW: 1.55, ST: 2.0,
};
/** Chance-creation propensity by position — chance a player lays on the assist. */
const ASSIST_WEIGHT: Partial<Record<Player['position'], number>> = {
...
/** Weighted index pick from `weights`; -1 if all zero. */
...
/**
 * Per-minute intensity multiplier from game state. Neutral (1) when level early — so the calibration
 * (identical squads) is untouched — then: a leading side eases off (more so with a bigger lead and
 * later in the game), a trailing side pushes, and a level game goes cagey in the closing minutes
 * (which nudges tight matches toward draws, the realistic version of a fitted low-score correlation).
 */
function stateFactor(myLead: number, minute: number): number {
  if (myLead === 0) return minute > 75 ? 0.88 : 1;
  const late = minute / 90;
  const mag = Math.min(Math.abs(myLead), 2);
  return myLead > 0 ? 1 - 0.16 * late * mag : 1 + 0.2 * late * mag;
}

function scoreGoal(side: 'home' | 'away', xi: Player[], minute: number): SimGoal | null {
  if (xi.length === 0) return null;
  const scorerIdx = weightedPick(xi.map((p) => (GOAL_WEIGHT[p.position] ?? 0.5) * (0.55 + 0.9 * (p.ratings.shooting / 100))));
  const scorer = xi[scorerIdx < 0 ? 0 : scorerIdx];
  let assistId: string | undefined;
  let assistName: string | undefined;
  if (Math.random() < ASSIST_SHARE) {
    const aw = xi.map((p, i) => (i === scorerIdx ? 0 : (ASSIST_WEIGHT[p.position] ?? 0.5) * (0.55 + 0.9 * (p.ratings.passing / 100))));
    const ai = weightedPick(aw);
    if (ai >= 0) { assistId = xi[ai].id; assistName = playerName(xi[ai]); }
  }
  return { minute, side, scorerId: scorer.id, scorerName: playerName(scorer), assistId, assistName };
}

/**
 * Play `minutes` of football from `startMinute`, returning the goals scored and the events. Home and
 * away are checked independently each minute (their per-minute intensity ≈ expected-goals/90), so the
 * total is ~Poisson(lambda) with no game state, and realistically skewed with it.
 */
function simulateEvents(
  homeXI: Player[], awayXI: Player[], lambdaHome: number, lambdaAway: number, minutes: number, startMinute: number,
): { homeGoals: number; awayGoals: number; goals: SimGoal[] } {
  const goals: SimGoal[] = [];
  let homeGoals = 0;
  let awayGoals = 0;
  const homePerMin = lambdaHome / 90;
  const awayPerMin = lambdaAway / 90;
  for (let t = 1; t <= minutes; t++) {
    const minute = startMinute + t;
    const lead = homeGoals - awayGoals;
    if (Math.random() < homePerMin * stateFactor(lead, minute)) {
      const g = scoreGoal('home', homeXI, minute);
      if (g) { goals.push(g); homeGoals += 1; }
    }
    if (Math.random() < awayPerMin * stateFactor(-lead, minute)) {
      const g = scoreGoal('away', awayXI, minute);
      if (g) { goals.push(g); awayGoals += 1; }
    }
  }
...
export function simulateMatch(
  ...
  // Displayed odds come from the closed-form model, independent of how this one match falls.
  const probabilities = computeOutcomeProbabilities(lambdaHome, lambdaAway);

  const reg = simulateEvents(homeXI, awayXI, lambdaHome, lambdaAway, 90, 0);
  let finalHome = reg.homeGoals;
  let finalAway = reg.awayGoals;
  ...
  if (requiresDecisiveResult && finalHome === finalAway) {
    if (eraRules.extraTimeMinutes > 0) {
      ...
      const et = simulateEvents(homeXI, awayXI, lambdaHome * (30 / 90), lambdaAway * (30 / 90), 30, 90);
